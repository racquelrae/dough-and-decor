  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={rowStyles.meta}>Qty: {item.quantity}{item.expires ? ` • exp ${item.expiryDate?.toDate?.()?.toLocaleDateString?.() ?? ""}` : ""}</Text>
      </View>
      <Pressable
        onPress={async () => {
          await updateItem(categoryId, item.id, { quantity: Math.max(0, (item.quantity ?? 0) - 1) });
