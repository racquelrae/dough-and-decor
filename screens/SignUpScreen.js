      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        <View style={theme.screenContainer}>
          ...
          <View style={theme.formContainer}>
            ...
            {error ? <Text style={theme.errorText}>{error}</Text> : null}
            {loading ? <ActivityIndicator size="small" color="#EDC7BA" style={{ marginVertical: 8 }} /> : null}
            <View style={theme.buttonRow}>
              <TouchableOpacity style={[theme.button, { flex: 1, marginHorizontal: 0 }]} onPress={handleSignUp} disabled={loading}>
                <Text style={theme.buttonText}>Sign up</Text>
              </TouchableOpacity>
            </View>
            <Text style={theme.alreadyHaveAccountText}>
              Already have an account? <Text style={theme.link} onPress={() => navigation.navigate('Login')}>Log In</Text>
            </Text>
          </View>
        </View>
      </ScrollView>
